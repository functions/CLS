 ...
 * 定位模块
 */
define("modules/map/geo", [ "./map" ], function(require, exports, module) {
    /**
     * 当前模块私有属性和方法
     */
    var _map = require("./map"), _geolocation = null, _posSucCalls = [], _posErrCalls = [], _watSucCalls = [], _watErrCalls = [], _watchNumber = -1, ERROR_INFO = {
    ...
    function onError(errData) {
        var i = 0, len = _posErrCalls.length;
        ...
        getInstance: function(callback) {
            if (_geolocation) {
                ...
                _map.plugin("AMap.Geolocation", function() {
                    ...
                    AMap.event.addListener(_geolocation, "complete", onComplete);
                    //返回定位信息
                    AMap.event.addListener(_geolocation, "error", onError);
                    //返回定位出错信息
                    callback && typeof callback === "function" && callback(_geolocation);
                });
            }
